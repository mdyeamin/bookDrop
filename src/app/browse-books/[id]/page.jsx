...
import { FiChevronRight, FiBookmark, FiEdit2, FiEyeOff, FiTrash2 } from 'react-icons/fi';
...
console.log(book)
  ...
  // Format the date (assuming book.createdAt is a valid date string or timestamp)
  ...
  const isAvailable = book.status !== 'checked_out';
  const statusColor = isAvailable ? 'bg-[#FF5A36]' : 'bg-slate-400';
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#F8FAFC] to-white py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-2 text-[13px] text-slate-500 mb-8 font-semibold">
          <Link href="/browse" className="hover:text-[#0A2540] transition-colors">Browse Books</Link>
          <FiChevronRight size={14} className="mt-0.5" />
          <span className="hover:text-[#0A2540] cursor-pointer transition-colors">{book.category || 'Category'}</span>
          <FiChevronRight size={14} className="mt-0.5" />
          ...
          <div className="lg:col-span-8 flex flex-col pt-2">
            ...
            <div className="flex flex-wrap items-center gap-4 text-base mb-8">
              <p className="text-slate-600">
                By <span className="font-bold text-[#103A62]">{book.author}</span>
              </p>
            ...
            <div className="text-slate-600 text-[15px] leading-[1.8] space-y-5 mb-10 pr-0 lg:pr-8">
              {descriptionParagraphs.length > 0 ? (
                descriptionParagraphs.map((para, idx) => <p key={idx}>{para}</p>)
              ) : (
            ...
            <div className="bg-[#F8FAFC] border border-slate-200 rounded-xl p-6 grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
              <div>
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">ISBN</p>
                <p className="text-sm font-semibold text-[#0A2540]">978-0-12-345678-9</p>
              </div>
              <div>
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Publisher</p>
                <p className="text-sm font-semibold text-[#0A2540]">LogisPress</p>
              </div>
              <div>
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Weight</p>
                <p className="text-sm font-semibold text-[#0A2540]">1.2 lbs / 540g</p>
              </div>
              <div>
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Location</p>
                <p className="text-sm font-semibold text-[#0A2540]">Zone A, Shelf 4</p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-4 mb-12">
              <form action={'/api/payment'}method='POST'>
                ...
                className={`text-white font-bold px-8 h-12 rounded-lg flex items-center gap-2.5 transition-colors shadow-sm ${
                  isAvailable ? 'bg-[#FA5D39] hover:bg-[#E54823]' : 'bg-slate-300 cursor-not-allowed'
                }`}
              >
            ...
            <div className="border-t border-slate-200 pt-6 mt-auto">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <Button size="sm" variant="bordered" className="bg-white border-slate-200 text-slate-600 hover:bg-slate-50 hover:border-slate-300 font-semibold h-9 px-4 rounded flex items-center gap-2">
                  <FiEdit2 size={13} />
                  Edit Details
                </Button>
                <Button size="sm" variant="bordered" className="bg-white border-slate-200 text-slate-600 hover:bg-slate-50 hover:border-slate-300 font-semibold h-9 px-4 rounded flex items-center gap-2">
                ...
                <Button size="sm" variant="bordered" className="bg-white border-slate-200 text-slate-600 hover:bg-red-50 hover:border-red-200 hover:text-red-600 font-semibold h-9 px-4 rounded flex items-center gap-2 transition-colors">
                  <FiTrash2 size={13} />
                  Delete Record
                </Button>
              </div>
